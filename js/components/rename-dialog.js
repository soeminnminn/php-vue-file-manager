...
export default {
  ...
  methods: {
    ...
    async handleSubmit(ev) {
      try {
        const res = await this.fetchApi('post', 'rename', this.fileOrDir, this.value);
    ...
    return h(DialogModal, { ref: 'dialog', modelValue: this.open, width: '20em', onShow: this.handleDialogShow, onClose: this.handleDialogClose }, {
      default: () => h('div', { class: 'dialog-content' }, [
        h('div', { class: 'dialog-body' }, 
          h(TextField, { ref: 'input', label: this.$t('Enter new name'), modelValue: this.value, onChange: this.handleTextChange })
        ),
